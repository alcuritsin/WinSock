...
extern int setup_for_ping(char* host, int ttl, SOCKET& sd, sockaddr_in& dest)
{
	...
	//Инициализируем host получателя
	...
	{
		//иначе, предпологаем что хост задан по имен и пытаемся разрешить имя в адрес.
		hostent* hp = gethostbyname(host);
		if (hp != 0)
		{
			//если имя разрешилось в IP адрес, записываем этот адрес в получателя.
...
extern void init_ping_packet(ICMPHeader* icmp_hdr, int packet_size, int seq_no)
{
	...
	//для того чтобы отличать ICMP-сообщение,
